...
<head>
	...
	function goPage(page){
		...
		$("#page").val(page);
		console.log("나는 frmSearch다" + $("#frmSearch").submit());
		$("#frmSearch").submit();
	}
	//조회 페이지 이동
...
<body>
	<div id="wrap">
    	...
    	<div class="rightBlock">
            ...
            <!-- 검색 시작 -->
            <form id="frmSearch" name="frmSearch" action="${_ctx}/board/doc/myList.java6" method="get"  class="search_area">
...
<%-- 	                            	<a href="javascript:goView('${item.docId}');">${item.title}</a> --%>
	                            	<a href = "${_ctx}/board/doc/view.java6?docId=${item.docId}&mapId=${item.mapId}">${item.title} </a><c:if test="${item.comments > 0}">[${item.comments}]</c:if></td>
